...
namespace API.Tests
{
    public class CategoryNodeControllerShould
    {
        ...
        [Fact]
        public async Task CreateCategoryNode_ReturnOk_WhenCategoryNodeWithExtraAttributesCreatedSuccessfully()
        {
            // Arrange
            SetupAdminUser();
            var parentId = Guid.NewGuid();
            ...
            var response = new CreateCategoryNodeResponse
            {
                ...
                IsActive = request.IsActive,
                SortOrder = request.SortOrder,
                CategoryMandatoryAttributes = new List<CategoryMandatoryAttributeResponseDto>
                {
                    new CategoryMandatoryAttributeResponseDto
                    {
                        Id = Guid.NewGuid(),
                        Name_en = "Wattage",
                        Name_fr = "Puissance",
                ...
                CategoryMandatoryExtraAttributes = new List<CategoryMandatoryExtraAttributeResponseDto>
                {
                    new CategoryMandatoryExtraAttributeResponseDto
                    {
                        Id = Guid.NewGuid(),
                        Name_en = "SKU",
                        Name_fr = "SKU",
            ...
            var okResult = Assert.IsType<OkObjectResult>(actionResult);
            Assert.Equal(StatusCodes.Status200OK, okResult.StatusCode);
            var returnedResult = Assert.IsType<Result<CreateCategoryNodeResponse>>(okResult.Value);
            Assert.True(returnedResult.IsSuccess);
            Assert.NotNull(returnedResult.Value);
            Assert.Equal(2, returnedResult.Value.CategoryMandatoryExtraAttributes.Count);
        }

        [Fact]
        public async Task BulkCreateCategoryNodes_ReturnOk_WhenNodesWithExtraAttributesCreatedSuccessfully()
        {
            // Arrange
            SetupAdminUser();
            var request = new BulkCreateCategoryNodesRequest
            ...
            var okResult = Assert.IsType<OkObjectResult>(actionResult);
            Assert.Equal(StatusCodes.Status200OK, okResult.StatusCode);
            var returnedResult = Assert.IsType<Result<BulkCreateCategoryNodesResponse>>(okResult.Value);
            Assert.True(returnedResult.IsSuccess);
            Assert.NotNull(returnedResult.Value);
            Assert.Equal(3, returnedResult.Value.TotalNodesCreated);
        }
    }
}
