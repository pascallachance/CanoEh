...
namespace API.Tests
{
    public class ItemServiceShould
    {
        ...
        [Fact]
        public async Task UpdateItemAsync_CallsUpdateAsyncOnRepository_WhenExistingVariantProvided()
        {
            // Arrange
            var variantId = Guid.NewGuid();
            var updateItemRequest = new UpdateItemRequest
            ...
            // Assert
            Assert.True(result.IsSuccess);
            _mockItemVariantRepository.Verify(
                x => x.UpdateAsync(It.Is<ItemVariant>(v => v.Id == variantId && v.StockQuantity == 50)),
                Times.Once);
        }

        [Fact]
        public async Task UpdateItemAsync_CallsAddAsyncOnRepository_WhenNewVariantProvided()
        {
            // Arrange
            var updateItemRequest = new UpdateItemRequest
            {
            ...
            _mockItemRepository.Setup(x => x.UpdateAsync(It.IsAny<Item>()))
                              .ReturnsAsync(existingItem);
            _mockItemVariantRepository.Setup(x => x.AddAsync(It.IsAny<ItemVariant>()))
                                      .ReturnsAsync((ItemVariant v) => v);
            ...
            Assert.True(result.IsSuccess);
            _mockItemVariantRepository.Verify(
                x => x.AddAsync(It.Is<ItemVariant>(v => v.Id == Guid.Empty && v.StockQuantity == 10)),
                Times.Once);
        }
