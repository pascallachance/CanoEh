...
namespace API.Tests
{
    public class TaxRatesServiceShould
    {
        ...
        [Fact]
        public async Task UpdateTaxRateAsync_ReturnSuccess_WhenRequestIsValid()
        {
            // Arrange
            var mockRepo = new Mock<ITaxRateRepository>();
            var service = new TaxRatesService(mockRepo.Object);
            var taxRateId = Guid.NewGuid();
            var existingTaxRate = new TaxRate
            {
                ID = taxRateId,
                Name_en = "GST",
                Name_fr = "TPS",
            ...
            mockRepo.Setup(repo => repo.ExistsAsync(taxRateId))
                   .ReturnsAsync(true);
            mockRepo.Setup(repo => repo.GetByIdAsync(taxRateId))
                   .ReturnsAsync(existingTaxRate);
            mockRepo.Setup(repo => repo.UpdateAsync(It.IsAny<TaxRate>()))
                   .ReturnsAsync((TaxRate taxRate) => { taxRate.UpdatedAt = DateTime.UtcNow; return taxRate; });

            // Act
            var result = await service.UpdateTaxRateAsync(updateRequest);

            // Assert
            Assert.True(result.IsSuccess);
            Assert.NotNull(result.Value);
            Assert.Equal("Updated GST", result.Value.Name_en);
            Assert.Equal(0.06m, result.Value.Rate);
            mockRepo.Verify(repo => repo.ExistsAsync(taxRateId), Times.Once);
            mockRepo.Verify(repo => repo.GetByIdAsync(taxRateId), Times.Once);
            mockRepo.Verify(repo => repo.UpdateAsync(It.IsAny<TaxRate>()), Times.Once);
        }

        [Fact]
        public async Task UpdateTaxRateAsync_ReturnFailure_WhenTaxRateDoesNotExist()
        {
            // Arrange
            var mockRepo = new Mock<ITaxRateRepository>();
            var service = new TaxRatesService(mockRepo.Object);
            ...
            Assert.Equal(StatusCodes.Status404NotFound, result.ErrorCode);
            mockRepo.Verify(repo => repo.ExistsAsync(taxRateId), Times.Once);
            mockRepo.Verify(repo => repo.UpdateAsync(It.IsAny<TaxRate>()), Times.Never);
        }

        [Fact]
        public async Task DeleteTaxRateAsync_ReturnSuccess_WhenTaxRateExists()
        {
            // Arrange
            var mockRepo = new Mock<ITaxRateRepository>();
            var service = new TaxRatesService(mockRepo.Object);
            ...
            mockRepo.Setup(repo => repo.GetByIdAsync(taxRateId))
                   .ReturnsAsync(taxRate);
            mockRepo.Setup(repo => repo.DeleteAsync(taxRate))
                   .Returns(Task.CompletedTask);

            // Act
            var result = await service.DeleteTaxRateAsync(taxRateId);

            // Assert
            Assert.True(result.IsSuccess);
            Assert.NotNull(result.Value);
            Assert.Equal(taxRateId, result.Value.ID);
            Assert.Contains("deactivated successfully", result.Value.Message);
            mockRepo.Verify(repo => repo.ExistsAsync(taxRateId), Times.Once);
            mockRepo.Verify(repo => repo.GetByIdAsync(taxRateId), Times.Once);
            mockRepo.Verify(repo => repo.DeleteAsync(taxRate), Times.Once);
        }

        [Fact]
        public async Task DeleteTaxRateAsync_ReturnFailure_WhenIdIsEmpty()
        {
            // Arrange
            var mockRepo = new Mock<ITaxRateRepository>();
            var service = new TaxRatesService(mockRepo.Object);
            ...
            // Act
            var result = await service.DeleteTaxRateAsync(Guid.Empty);
